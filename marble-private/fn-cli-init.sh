...
# Note: some transactions can be done by only one of the orgs, and so the endorsement policy uses 'OR' for simplicity here;
# real production should use `AND` for most transactions; some transactions may use a different endorsement policy by packaging it as a second chaincode.
CC_END_POLICY="OR ('Org1MSP.peer','Org2MSP.peer')"
CC_COLL_CONFIG=${CC_PATH}/marbles02_private/collections_config.json
